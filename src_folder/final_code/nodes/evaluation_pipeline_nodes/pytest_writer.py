...
def pytest_writer(state: CodeEvalState):
    ...
    utgenerated = generate_ut_llm_call(use_cases, python_code, mock_tools_code)
...
def generate_ut_llm_call(use_cases, python_code, mock_tools_code):
    pytest_llm = get_model(ModelName.GEMINI25FLASH).with_structured_output(UtGeneration)
    utgenerated: UtGeneration = pytest_llm.invoke([HumanMessage(content=PYTEST_WRITER_PROMPT.format(python_code=python_code, use_cases=use_cases, mock_tools_code=mock_tools_code, FINAL_RESPONSE_STR=FINAL_RESPONSE_STR, TRAJECTORY_STR=TRAJECTORY_STR))])
    return utgenerated
