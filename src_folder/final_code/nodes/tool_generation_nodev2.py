...
def get_composio_tools_node(state: AgentBuilderState) -> Command[Literal["process_non_composio_tools", "__end__"]]:
    try:
        ...
        return Command(
            goto="process_non_composio_tools",
            update={"tool_set": tool_set, "json_schema": json_schema, "agent_status_list": state["agent_status_list"]}
        )
        ...
        return Command(
            ...
            update={
                ...
                "messages": [AIMessage(content="An error occurred during selecting Composio tools. Please try again.")]
            }
        )


async def process_non_composio_tools(state: AgentBuilderState) -> Command[Literal["generate_tools_code", "__end__"]]:
    try:
        json_schema: JSONSchema = state["json_schema"]
        tool_list = ""
        ...
        if tool_list == "":
            return Command(
                goto="generate_tools_code",
                update={"agent_status_list": state["agent_status_list"]}
            )
        new_state = state
        ...
        return Command(
            goto="generate_tools_code",
            update={"agent_status_list":state["agent_status_list"] ,"json_schema": updated_json_schema["json_schema"], "messages": updated_json_schema["messages"]}
        )
    except Exception as e:
        if isinstance(e, GraphInterrupt):
            raise
        return Command(
            ...
            update={
                "exception_caught": f"{e}\n{traceback.format_exc()}",
                "messages": [AIMessage(content="An error occurred during processing custom tools. Please try again.")]
            }
        )
...
"""
        ...
        return Command(
            ...
            update={
                "exception_caught": f"{e}\n{traceback.format_exc()}",
                "messages": [AIMessage(content="An error occurred during generating tools code. Please try again.")]
            }
        )