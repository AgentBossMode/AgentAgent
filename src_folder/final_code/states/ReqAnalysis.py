...
class AgentAction(BaseModel):
    ...
    action: str = Field(description="Ex format: agent uses the x tool to do 'xyz' OR the llm analyzes/extracts etc etc...")
...
class DryRun(BaseModel):
    ...
    confident: bool = Field(description="Set true if high confidence that this dry_run is intended given the input")
    selected: bool = Field(default=False)

class DryRuns(BaseModel):
    dry_runs: List[DryRun] = Field(description="A list of dry runs generated for the agent")
    user_selections:dict = Field(default={}, description="Donot populate this, let it be default")
